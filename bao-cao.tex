...
\newcommand{\HueLogoFile}{logo_hueuni.png} % cùng thư mục với .tex (Overleaf: upload logo_hueuni.png vào gốc project)
% =============================================================================
% ========================  NỘI DUNG TÀI LIỆU  ================================
% =============================================================================
\begin{document}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        font=\scriptsize,
        >=Latex,
        mqttn/.style={draw=black!55, rounded corners=2pt, fill=green!14, inner sep=6pt, align=center, minimum width=2.9cm},
        fn/.style={draw=black!55, rounded corners=2pt, fill=yellow!12, inner sep=5pt, align=center, text width=2.15cm},
        uin/.style={draw=black!55, rounded corners=2pt, fill=blue!9, inner sep=5pt, align=center, text width=2.35cm},
    ]
        \node[mqttn] (m1) {\textbf{mqtt in}\\\texttt{iot/env/data}};
        \node[fn, text width=8.5cm, minimum height=1.1cm] (hd) at (0,-2.1) {\textbf{HandleData} (function)\\[2pt]parse JSON, tách 4 nhánh song song};
        \draw[->, thick] (m1) -- (hd);
        \foreach \x/\lab in {-4.2/temperature, -1.4/humidity, 1.4/alert, 4.2/ledState}
            \node[fn] (\lab) at (\x,-4.2) {\textbf{\lab}};
        \coordinate (S) at (0,-2.95);
        \draw[->, thick] (hd.south) -- (S);
        \foreach \lab in {temperature, humidity, alert, ledState}
            \draw[->, thick] (S) |- (\lab.north);
        \node[fn] (tf) at (-4.2,-6.1) {temperature\\function};
        \node[fn] (hf) at (-1.4,-6.1) {humidity\\function};
        \node[fn] (af) at (1.4,-6.1) {alert\\function};
        \node[fn] (lf) at (4.2,-6.1) {ledState\\function};
        \foreach \a/\b in {temperature/tf, humidity/hf, alert/af, ledState/lf}
            \draw[->, thick] (\a) -- (\b);
        \node[uin] (u1) at (-4.2,-8.2) {\textbf{ui\_gauge}\\\textbf{ui\_chart}};
        \node[uin] (u2) at (-1.4,-8.2) {\textbf{ui\_gauge}\\\textbf{ui\_chart}};
        \node[uin] (u3) at (1.4,-8.2) {\textbf{ui\_toast}};
        \node[uin] (u4) at (4.2,-8.2) {\textbf{ui\_switch}};
        \draw[->, thick] (tf) -- (u1);
        \draw[->, thick] (hf) -- (u2);
        \draw[->, thick] (af) -- (u3);
        \draw[->, thick] (lf) -- (u4);
