...
Trước hết, nhóm sinh viên xin gửi lời cảm ơn chân thành đến \textbf{GV Lê Hữu Bình} --- giảng viên hướng dẫn môn \textit{Phát triển ứng dụng IoT} --- đã tận tình giảng dạy, cung cấp kiến thức nền tảng về IoT, MQTT, Node-RED, và hướng dẫn chi tiết trong suốt quá trình thực hiện đề tài. Những góp ý của thầy đã giúp nhóm hoàn thiện thiết kế hệ thống từ phần cứng đến phần mềm.

Xin cảm ơn \textbf{Khoa Công nghệ thông tin, Trường ĐH Khoa học Huế} đã tạo điều kiện về phòng thí nghiệm, thiết bị và môi trường học tập để nhóm hoàn thành dự án.

Xin cảm ơn các bạn trong lớp đã trao đổi kinh nghiệm, chia sẻ tài liệu tham khảo và động viên trong quá trình thực hiện.

Cuối cùng, xin gửi lời cảm ơn đến gia đình và bạn bè --- những người đã luôn ủng hộ, động viên để nhóm có đủ nghị lực hoàn thành đề tài đúng tiến độ.
...
\textbf{File gốc (có chú thích):} \file{iot-nhom6/iot-nhom6.ino}.\\
\textbf{Bản dưới đây:} toàn bộ mã đã \textbf{gỡ chú thích} (\texttt{//} và \texttt{/* */}) để phụ lục gọn; sinh tự động từ file gốc bằng \file{tools/strip\_ino\_comments.py} $\rightarrow$ \file{appendix-iot-nhom6-nocomment.ino}.

Nội dung gồm: chân GPIO, Wi-Fi, MQTT, NTP, callback, OLED, DHT, LED PWM, lịch tự động, \code{setup()}/\code{loop()}.

\begin{mdframed}[backgroundcolor=yellow!15, linecolor=yellow!55]
    \textbf{Lưu ý khi nộp/công bố:} Trong mã có thể còn SSID, mật khẩu Wi-Fi, MQTT --- nên thay bằng \code{****} trong bản nộp công khai (hoặc chỉ nộp bản đã che).
\end{mdframed}

\IfFileExists{appendix-iot-nhom6-nocomment.ino}{%
  \lstinputlisting[
    style=appendixcode,
    caption={Firmware ESP8266 (đã gỡ chú thích, 487 dòng)},
  ...
  \begin{mdframed}[backgroundcolor=yellow!20, linecolor=yellow!60]
    ...
    \code{python tools/strip\_ino\_comments.py}\\[0.4em]
...
\textbf{Không có mã nguồn web HTML/JS/CSS riêng} --- giao diện Dashboard hoàn toàn do \textbf{NRD} sinh ra từ flow JSON.

\textbf{Toàn bộ mã JSON export} (đồng bộ với \file{node-red.json} trong project):

\IfFileExists{node-red.json}{%
  \lstinputlisting[
    style=appendixjson,
    caption={Node-RED --- export flow (JSON, 637 dòng)},
    label={lst:appendix-nodered}
  ]{node-red.json}%
}{%
  \begin{mdframed}[backgroundcolor=yellow!20, linecolor=yellow!60]
    \textbf{Lỗi:} Không tìm thấy \file{node-red.json}. Đặt file này cùng thư mục với \file{bao-cao.tex}.
  \end{mdframed}
}
